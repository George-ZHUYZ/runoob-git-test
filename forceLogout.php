...
/*
 * This file get the functions which are used to force logout last time and login
 */
...
if (empty($newResult)) {
    $fieldValues = array($allResult[0][1], $allResult[0][0], date("Y-m", time()), $loginTime . " " . $loginIP . "<#>Force-Logout<*>" . $loginTime . "" . $loginIP . "<#>Login<*>");
    insertData("LoginHistory", $fieldValues);
} else {
    updateData("LoginHistory", $newResult[0][0], array("LoginRc"), array("string"), array($newResult[0][4] . $loginTime . " " . $loginIP . "<#>Force-Logout<*>" . $loginTime . "" . $loginIP . "<#>Login<*>"));
}
